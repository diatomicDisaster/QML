...
class QAUM(QuantumModel):
    ...
    def model(self, weights, features, qbits):
        ...
        for i in range(self.depth):
            for j in range(self.features_per_qbit):
                ...
                w = (
                    (self.n_local*self.nqbits) # weights per trainable layer
                    * (self.features_per_qbit*i + j) # no. of previous trainable layers
                    + (self.n_local)*self.nqbits # zero-th weight layer
                )
    ...
    def initialise_weights(self, seed=None):
        ...
        n_train_layer = self.features_per_qbit * self.nqbits * self.n_local
        n = n_train_layer*self.depth + self.nqbits*self.n_local
...
def train(model, optimizer, data, num_epochs=150):
    ...
    for i in range(num_epochs):
        ...
        np.savetxt("train_weights.csv", train_weights, delimiter=',')
        np.savetxt("train_losses.csv", train_losses, delimiter=',')
        print(f"Epoch {i} loss: {train_losses[i]:5.3f}  Time taken: {end - start:4.1f}s")
    np.savetxt("train_losses.csv", train_losses, delimiter=',')
    np.savetxt("train_weights.csv", train_weights, delimiter=',')
    return weights

def validate(model, weights, data):
    errors = 0
...
train_X, validate_X, train_Y, validate_Y = fetch_data_random_seed_val(300, 0)
...
#weights = train(model, opt, (train_X, train_Y), num_epochs=150)
weights = np.loadtxt("train_weights.csv", delimiter=",")[-1]
acc = validate(model, weights, (train_X, train_Y))
print(acc)